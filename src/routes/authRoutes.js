...
router.post('/register', [
  ...
  body('academic_role').isIn(['mahasiswa', 'dosen', 'tendik']).withMessage('Invalid academic role'),
  ...
  try {
    ...
    const { nama, username, email, password, academic_role, no_induk } = req.body;
    ...
    const result = await executeQuery(
      ...
      [nama, username, email, hashedPassword, academic_role, no_induk]
    );
...
router.post('/login', [
  ...
  try {
    ...
    res.json({
      success: true,
      message: `${accountType.charAt(0).toUpperCase() + accountType.slice(1)} login successful`,
      data: responseData,
      token
    });
